...
<<BROM>> and Preloader will authenticate the <<DA>> before executing it.
...
[[DEVAPC]]DEVAPC:: Device Access Permission Control.

[[DPM]]DPM:: DRAM Power Manager.

[[DSP]]DSP:: Digital Signal Processor.

[[GCE]]GCE:: Global Command Engine. A SoC peripheral that can be used to
program registers with strict timing requirements. It has 64-bit instruction
set and a few major revisions with new encoding and more complex instructions.
...
[[MCUPM]]MCUPM:: MCUSYS Power Management.
...
[[SBC]]SBC:: Secure Boot Chain. When this is enabled, the BROM will only load
and run properly signed boot code.
...
[[SLA]]SLA:: Serial Link Authentication. There's 3 types of the SLA: BootROM
SLA, Preloader SLA and DA SLA. When BROM SLA is enabled, host must query unique
SoC identifier and sign it with a private key. The public key is fused into the
SoC and will be used to verify signed data. The SLA challenge must be passed in
order to boot the DA. Preloader SLA is used only by certain vendors for
additional security. DA SLA depends on DA build configuration and doesn't
depend on the SoC SLA fuse.
